...
class rand(Player):
    ...
    def placeBets(self):
        self.initialBet = random.randint(0, self.stake)
        super().placeBets()
...
rsim = Simulator(rand(rtable), rgame)
maxima, duration = rsim.gather()
print(maxima)
print(duration)
print(maxima[duration.index(max(duration))])
print(duration[maxima.index(max(maxima))])
print(maxima.mean())
print(duration.mean())